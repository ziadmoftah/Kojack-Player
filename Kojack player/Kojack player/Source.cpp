...
#define Imgs_directory "C:\\Users\\MhmdAdnan\\source\\repos\\FINALONE\\FINALONE\\Imgs/"
...
int main() {
	...
	sf::Font myfont;
	myfont.loadFromFile(Imgs_directory"Roboto-ThinItalic.ttf");
...
void song_tab(sf::RenderWindow& window, sf::Vector2f& mouse_position, sf::Event& event, string& mod, sf::Music& music, bool& playing) {
	...
	while_playing.search_box.setFillColor(sf::Color::White);
	while_playing.searching_pic.loadFromFile(Imgs_directory"Searh icon.png");
	while_playing.search_box.setTexture(&while_playing.searching_pic);
...
void searching_window() {
	...
	CircleShape done_button_search;
	Texture done_button_search_texture;
	done_button_search_texture.loadFromFile(Imgs_directory"search bar button.png");
	///////////////////////////////////////////////////////////////////////////////////
	...
	search_text.setPosition(180, 50);
	...
	///////////// button done search ///////////
	done_button_search.setRadius(15);
	done_button_search.setPosition(130, 53);
	done_button_search.setFillColor(Color::White);
	done_button_search.setTexture(&done_button_search_texture);
	/////////////////////////////////////////////

	
	///// button title ////
	title_name_button_search[0].setString("  Name");
	title_name_button_search[1].setString("Album");
	title_name_button_search[2].setString("artist");
	title_name_button_search[3].setString("year");
	float x = 5;
	for (int i = 0; i < 4; ++i) {
		title_button_search[i].setSize(Vector2f(90, 40));
		title_button_search[i].setPosition(x, 10);
		title_button_search[i].setFillColor(Color::Transparent);
		title_name_button_search[i].setFont(use_font);
		title_name_button_search[i].setStyle(Text::Bold);
		title_name_button_search[i].setPosition(x, 10);
		title_name_button_search[i].setCharacterSize(32);
		x += 130;
	...
	while (Searchwindow.isOpen())
	{
		...
		while (Searchwindow.pollEvent(starting_search)) {
			...
			if (focus(done_button_search.getGlobalBounds(), mouse_position)) {
			...
			for (int i = 0; i < 4; i++) {
				if (focus(title_button_search[i].getGlobalBounds(), mouse_position)) {
					if (starting_search.type == sf::Event::MouseButtonPressed  && starting_search.mouseButton.button == sf::Mouse::Left) {
						{
							searchicator = i;
				...
				/////// عشان ياخد نفس كيمة1 الي هنا         drawتقولي حتطيت ليه /////////////
				if (searchicator == i) {
					title_name_button_search[i].setFillColor(sf::Color::Black);
				}
				else {
					title_name_button_search[i].setFillColor(sf::Color::Blue);
				}
			//////////////////////////////////////////////////////////////////
			}
				////////////////////////////////////////////////////////////////
			...
			for (int i = 0; i < searching_results.size(); ++i) {
				if (focus(text_searching_results[i].getGlobalBounds(), mouse_position)) {
					if (starting_search.type == sf::Event::MouseButtonPressed  && starting_search.mouseButton.button == sf::Mouse::Left) {
						playMusic(searching_results[i], i, playing, music);
		...
		for (int i = 0; i < 4; i++) {
			Searchwindow.draw(title_button_search[i]);
			Searchwindow.draw(title_name_button_search[i]);
		...
		Searchwindow.draw(done_button_search);
